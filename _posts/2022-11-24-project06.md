...
- 따라서 아래와 같이 <span style="color:red">빨간점</span>에서부터 4개의 기준 좌표들까지의 거리를 Nkm가 아닌 N√2로 잡고 Polygon을 생성한 후, 그 범위 내에서 Nkm안에 포함되는 좌표들을 구해야 한다. 즉, r=N인 원의 범위가 최종적으로 우리가 찾고자 하는 공간 데이터의 탐색 범위가 될 것이다. (피타고라스의 정리)
