...
- Geolatte는 WKT를 활용하지 않고 Geometry를 생성할 수 있는 DSL(도메인 특화 언어) Class를 가지고 있
...
- 기존의 거리 기반 매장 탐색 쿼리를 개선하고자 **Hibernate Spatial**을 사용하기로 함.
- 기존의 쿼리보다 보다 성능이 나은지를 기대하고 테스트를 진행.

## 디테일한 테스트 설정

- 목적
  - **현재 중심 좌표에서 Nkm 이내에 존재하는 모든 매장의 정보**를 얻고자 함
- 구분
  1. 모든 테이블 데이터를 조회한 후, 현재 좌표와 매장 좌표 간의 거리를 구하여 이 값이 N보다 작은 경우
  2. MBRContains를 활용한 경우
  3. ST_Contains (혹은 ST_Within)을 활용한 경우 (둘의 성능 차이는 없음)
- 세팅
  - MBRContains
- 테스트 데이터
  - 총 1천만 111개
  - 101만개, 901만개, 109개 - 3개의 그룹으로 나누어짐. **(즉, Point 주소는 총 111개밖에 없음)**
    - 109개 (id 범위: 10000000 이하의 숫자 109개)
    - 101만개 (id 범위: 12000000 ~ 13000000)
    - 901만개 (id 범위: 10000000 ~ 20000000 중 이전에 나온 id를 제외한 나머지)
